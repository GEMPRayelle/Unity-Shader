Shader "Unlit/steep_parallax_mapping"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float2 ParallaxOffset(float2 uv, float3 viewDirTS)
            {
                ...
                float2 P = viewDirTS.xy * _HeightScale * -1;
                ...
                for(int j = 0; j < 32; ++j)
                {
                    ...
                    currentDepthMapValue = SAMPLE_TEXTURE2D(_HeightMap, sampler_HeightMap, currentTexCoords).r;
                ...
                float beforeDepth = SAMPLE_TEXTURE2D(_HeightMap, sampler_HeightMap, prevTexCoords).r - currentLayerDepth + layerDepth;
            ...
            float4 frag (Interpolators i) : SV_TARGET 
            {
                ...
                //Parallax Mapping된 텍스처 가장자리에 어색한 경계면을 제거
