    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag (Interpolators i) : SV_TARGET 
            {
                ...
                //Refraction
                float3 refractDirR = refract(-V, N, 0.65);
                float3 refractDirG = refract(-V, N, 0.67);
                float3 refractDirB = refract(-V, N, 0.68);

                //Chromatic Aberration
                float3 refr_R = SAMPLE_TEXTURECUBE(_Cube, sampler_Cube, refractDirR).rbg;
                float3 refr_G = SAMPLE_TEXTURECUBE(_Cube, sampler_Cube, refractDirG).rbg;
                float3 refr_B = SAMPLE_TEXTURECUBE(_Cube, sampler_Cube, refractDirB).rbg;

                float3 refractionColor_Chromatic = float3(refr_R.r, refr_G.g, refr_B.b);
                ...
                float3 color = lerp(refractionColor_Chromatic, reflectionColor, f);
